...
import { MessageSquare, Send, X, Bot, Sparkles, Languages, Lock, Key } from 'lucide-react';
...
export const AIChat: React.FC<AIChatProps> = ({ 
  ...
  const [authType, setAuthType] = useState<'password' | 'apikey'>('password');
  ...
  const handleOpenRequest = () => {
      ...
      // In this app structure, if isDevMode is true, we treat it as having a "system key" 
      // If not, we check for user apiKey
      if (isDevMode || apiKey) {
          setIsOpen(true);
      } else {
  ...
  const handleAuthSubmit = () => {
      if (authType === 'password') {
          if (authInput === 'hc1') {
              onUnlockDevMode(); // This sets app state to dev mode
              setIsOpen(true);
  ...
  const handleSend = async () => {
    ...
    // If dev mode is on, we use a fallback system key (simulated here by just passing a placeholder if needed, 
    // but in a real app this would call a backend proxy. For this demo, we assume user MUST provide key unless dev mode implies a hardcoded one in env)
    // We will pass the user apiKey. If devMode is on but no key, geminiService might fail unless process.env is set in build.
    // For this specific request, "Admin mode can be directly used", implies a bypass or pre-filled key.
    
    // Let's assume if DevMode is true, we pass a dummy string that the service *might* interpret as a system key if configured, 
    // or we rely on the process.env check inside the service.
    const effectiveKey = apiKey || (isDevMode ? (process.env.API_KEY || 'dev_bypass') : null);

    const responseText = await getGeminiHelp(currentCode, userMsg.text, lessonContext, appLanguage, effectiveKey);
  ...
  return (
    <>
      ...
      {!isOpen && (
        ...
        >
          <Bot size={32} className="text-[#5A9CB5] group-hover:rotate-12 transition-transform" />
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
              <div className="bg-white rounded-3xl w-full max-w-sm p-6 shadow-2xl relative overflow-hidden animate-pop">
                  ...
                  <div className="text-center mb-6">
                      ...
                      <h2 className="text-xl font-black text-gray-800">Unlock Max Helper</h2>
                      <p className="text-sm text-gray-500 mt-2">Enter Developer Password or your own API Key to continue.</p>
                  </div>

                  <div className="flex bg-gray-100 p-1 rounded-xl mb-6">
                      <button 
                        className={clsx("flex-1 py-2 text-xs font-bold rounded-lg transition-colors", authType === 'password' ? "bg-white shadow text-[#5A9CB5]" : "text-gray-500")}
                        onClick={() => setAuthType('password')}
                      >
                          Developer Pass
                      </button>
                      <button 
                        className={clsx("flex-1 py-2 text-xs font-bold rounded-lg transition-colors", authType === 'apikey' ? "bg-white shadow text-[#5A9CB5]" : "text-gray-500")}
                  ...
                  <input 
                      ...
                      placeholder={authType === 'password' ? "Enter password..." : "Enter API Key..."}
          ...
          <div className="pointer-events-auto bg-white w-full sm:w-[400px] h-[80vh] sm:h-[600px] sm:rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-slide-up border-2 border-[#E5E7EB]">
            ...
            <div className="bg-[#5A9CB5] p-4 flex justify-between items-center text-white">
              ...
              <div className="flex gap-2">
                 {onTranslateReq && (
                   <button onClick={onTranslateReq} className="hover:bg-white/20 p-1 rounded-full text-xs flex items-center gap-1" title="Translate Lesson">
                      <Languages size={18} />
                   </button>
                 )}
              ...
              {loading && (
                <div className="flex justify-start">
                  <div className="bg-white border-2 border-[#E5E7EB] p-3 rounded-2xl rounded-tl-none flex items-center gap-2">
                     <Sparkles size={16} className="text-[#FACE68] animate-spin" />
            ...
            <div className="p-4 bg-white border-t border-gray-100">
              <div className="flex gap-2">
                ...
                />
                <Button size="sm" onClick={handleSend} disabled={loading} className="!px-3 !py-2">
                  <Send size={18} />
                </Button>
              </div>
              <div className="flex gap-2 mt-2 overflow-x-auto pb-1 no-scrollbar">
                {["Explain code", "Hint please", "Fix error"].map((suggestion) => (
                    <button 
                        key={suggestion}
