...
import { Settings, Globe, Sun, Key, Lock, CheckCircle } from 'lucide-react';
...
export const SettingsView: React.FC<SettingsViewProps> = ({ user, theme, t, onUpdateSetting, onUpdateApiKey }) => {
        ...
        <div className="flex-1 p-6 md:p-12 overflow-y-auto pb-24" style={{ backgroundColor: themeStyles.bg, color: themeStyles.text }}>
            ...
            <div className="space-y-6">
                <Card theme={theme} className="p-8">
                    <h3 className="text-xl font-bold mb-6 flex items-center gap-2 border-b pb-2"><Globe size={24} className="text-[var(--color-primary)]"/> {t.selectLanguage}</h3>
                ...
                <Card theme={theme} className="p-8">
                     <h3 className="text-xl font-bold mb-6 flex items-center gap-2 border-b pb-2"><Sun size={24} className="text-[var(--color-secondary)]"/> {t.selectTheme}</h3>
                     <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                         {(['light', 'dark', 'midnight', 'candy', 'orange', 'forest', 'coffee', 'golden', 'neon', 'berry', 'lemon', 'cosmic', 'aqua'] as Theme[]).map(th => (
                             <button 
                                key={th}
                             ...
                             >
                                 <div className="relative z-10 flex flex-col items-center gap-2">
                                     <div className="w-6 h-6 rounded-full border border-white/20 shadow-sm" style={{backgroundColor: THEMES[th].primary}}></div>
                ...
                <Card theme={theme} className="p-8">
                     <h3 className="text-xl font-bold mb-6 flex items-center gap-2 border-b pb-2"><Key size={24} className="text-[var(--color-accent)]"/> AI Assistant Key</h3>
                     <p className="text-sm text-gray-500 mb-4 bg-gray-50 p-3 rounded-lg border border-gray-100">Enter your API key to enable AI coding tutorials.</p>
                     ...
                     {user.adminUnlocked && <p className="text-xs text-green-600 bg-green-50 p-2 rounded mt-2 font-bold flex items-center gap-1"><CheckCircle size={12}/> Developer Mode Unlocked (System Key Active)</p>}
                </Card>
